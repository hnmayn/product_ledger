...
public class FrontController extends HttpServlet {
     ...
     * (例：パラメータがaction=User の場合、actions.UserActionオブジェクト)
     * @param request リクエスト
    ...
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            // リクエストからパラメータ"action"の値を取得(例："User","Product")
            String actionString = request.getParameter(ForwardConst.ACT.getValue());

            // 該当するActionのオブジェクトを作成(例：リクエストからパラメータ action=User の場合、actions.UserActionオブジェクト
